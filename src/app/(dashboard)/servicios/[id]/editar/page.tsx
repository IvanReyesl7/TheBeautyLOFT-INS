...
export default function EditarServicio() {
  ...
  useEffect(() => {
    async function fetchServicio() {
      try {
        ...
        setErrors(['No se pudo cargar el servicio']);
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error:', error);
      setErrors([error.message || 'Error al actualizar el servicio']);
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center space-x-3">
            <input
              ...
              className="w-5 h-5 text-black border-gray-300 rounded focus:ring-2 focus:ring-gray-400"
            />
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-black text-white py-3 px-4 rounded-lg font-semibold hover:bg-gray-800 transition disabled:opacity-50"
            >
